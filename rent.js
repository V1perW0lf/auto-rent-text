...
const server = http.createServer((request, response) => {
  ...
  if (request.method === "POST" && request.url === "/calc/text") {
    ...
    request.on('data', (chunk) => {
      ...
      const chargeAmountBegInd = body.indexOf('Total Amount:') + 52 //For putsmail, 51 works perfectly
      const chargeAmountEndInd = chargeAmountBegInd + 9; //For putsmail, 8 works perfectly
